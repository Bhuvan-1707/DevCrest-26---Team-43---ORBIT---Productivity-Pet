...
import { X, Activity, Clock, Compass, Layers, CheckCircle2, FileText, Code } from 'lucide-react';
      ...
      {observation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 select-none">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-lg overflow-hidden rounded-2xl border border-slate-800 bg-[#0d101a] p-6 shadow-2xl shadow-cyan-950/30"
          >
            {/* Top Accent Line */}
            ...
            <div className="py-5 flex flex-col gap-4 text-xs">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800/60 font-mono text-[11px]">
                <div className="flex items-center gap-2">
                  <Code size={13} className="text-slate-400" />
                  <span className="text-slate-300 font-bold">{observation.type}</span>
                </div>
              ...
              {/* 1. Activity Section */}
              <div className="flex flex-col gap-2 p-3.5 rounded-xl bg-slate-900/40 border border-slate-800/50">
                <div className="flex items-center gap-1.5 text-cyan-400 font-bold font-heading text-[11px]">
                  <Layers size={13} />
                  <span>ACTIVITY</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-slate-300">
                  <div>
                    ...
                    <span className="font-semibold text-slate-100">{observation.activity?.name || 'N/A'}</span>
                  </div>
                  <div>
                  ...
                  {observation.activity?.duration > 0 && (
                    <div className="col-span-2 mt-1 pt-1.5 border-t border-slate-800/40">
              ...
              {/* 2. Context Section */}
              <div className="flex flex-col gap-2 p-3.5 rounded-xl bg-slate-900/40 border border-slate-800/50">
                <div className="flex items-center gap-1.5 text-indigo-400 font-bold font-heading text-[11px]">
                  <Compass size={13} />
                  <span>CONTEXT</span>
                </div>
                <div className="flex flex-col gap-1.5 font-mono text-[11px] text-slate-300">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-200">{observation.context?.page || '/'}</span>
                  </div>
                  {observation.context?.taskId && (
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-sans">Task ID:</span>
                      <span className="text-slate-300">{observation.context.taskId}</span>
                    </div>
                  )}
                  {observation.context?.sessionId && (
                    <div className="flex items-center justify-between">
                      <span className="text-slate-400 font-sans">Session ID:</span>
                      <span className="text-slate-300">{observation.context.sessionId}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {observation.metadata && Object.keys(observation.metadata).length > 0 && (
                <div className="flex flex-col gap-2 p-3.5 rounded-xl bg-slate-900/40 border border-slate-800/50">
                  <div className="flex items-center gap-1.5 text-emerald-400 font-bold font-heading text-[11px]">
                    <FileText size={13} />
                    <span>METADATA</span>
                  </div>
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-400 font-mono">
              ...
              <button
                onClick={onClose}
                className="px-3 py-1.5 rounded-lg bg-slate-800 text-slate-200 font-bold font-heading hover:bg-slate-700 transition-colors"
              >
                Close
              </button>
            </div>
