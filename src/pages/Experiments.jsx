    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full select-none">
      ...
      {showAddForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="orbit-card p-6 border-amber-500/30">
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 font-heading mb-4">Design New Experiment</h3>
            <form onSubmit={handleCreateExperiment} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Experiment Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                    ...
                    className="w-full bg-white dark:bg-[#07090e] border border-slate-300 dark:border-slate-700/80 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Tested Variable</label>
                  <input
                    type="text"
                    value={variable}
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Hypothesis</label>
                <textarea
                  rows={2}
                  value={hypothesis}
      ...
      {loading ? (
        ...
        <Card className="orbit-card p-12 text-center flex flex-col items-center">
          <Zap size={32} className="text-slate-400 dark:text-slate-600 mb-3" />
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">No active experiments</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm">Click 'New Experiment' to design your first productivity test.</p>
        </Card>
      ) : (
          ...
          {experiments.map(exp => (
            <Card key={exp.id} className="orbit-card p-6 flex flex-col justify-between relative overflow-hidden group">
              <div className="flex flex-col gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 font-heading mt-1">{exp.name}</h3>
                {exp.variable && <span className="text-[10px] font-semibold text-sky-600 dark:text-cyan-400 uppercase tracking-wider">Variable: {exp.variable}</span>}
                {exp.hypothesis && <p className="text-xs text-slate-600 dark:text-slate-300 italic">"{exp.hypothesis}"</p>}
              </div>

              <div className="mt-4 pt-3 border-t border-slate-200/80 dark:border-slate-800/60 flex items-center justify-between text-xs">
                <span className="text-slate-500 dark:text-slate-400">Result: <strong className="text-slate-800 dark:text-slate-200">{exp.result || 'In Progress'}</strong></span>
                <button
                  onClick={() => handleCompleteExperiment(exp)}
                  className="px-3 py-1 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 hover:bg-amber-100 dark:hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                >
                  <Check size={13} />
